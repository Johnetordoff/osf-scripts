...
def main():
    ...
    target_node_id = "pr35b"  # Replace with the actual node ID
    message_recipent_id = "uhgtw"
    requested_permissions = "write"
    comment = "Requesting institutional access to this node."
    institution_id = "osftype0"  # Replace with the actual institution ID

    try:
        logging.info(f"Creating an InstitutionalRequest for node ID: {target_node_id} and institution: {institution_id}")
        response = create_institutional_access_request(env, target_node_id, comment, institution_id, token, message_recipent=message_recipent_id, requested_permissions=requested_permissions)
